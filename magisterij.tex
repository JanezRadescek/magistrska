...
\begin{figure}[tb]
	\parbox{\textwidth}{
		...
		\bnfis & A_\op \,\bnfor\! \tyfun{X}{Y} \,\bnfor\! \typromise{X}
		\\
		\text{Tip izračuna $I$} \bnfis& \tycomp{X}
		\\
		\text{Tip procesa \tyC, \tyD}  \bnfis & \tyrun{X} \,\bnfor\! \typar{\tyC}{\tyD}
		\end{align*}
	} 
	\caption{Tipi}
	\label{fig:tipi}
...
\begin{figure}[tp]
	...
	\begin{mathpar}
		...
		\coopinfer{Tip-Fun}{
			...
			\Gamma \types \tmfun{x : X}{M} : \tyfun{X}{Y}
		}
		\quad
		\coopinfer{Tip-Fun-Rec}{
			\Gamma, x \of X \types M : Y
		}{
			\Gamma \types \tmfunrec{f}{x : X}{M} : \tyfun{X}{Y}
		}
	\end{mathpar}
