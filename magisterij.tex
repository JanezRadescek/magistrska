...
%spawn = razplod?? meni je lepše razcep/ ??
%interpreter = tolmač
...
%thread = nit
%payload = tovor
%Ground type = prizemljen/osnovni/temeljni??
%mobile = mobilen
%box/boxed = okvir/okvirjen     /škatla/paket ??
%recursion = rekurzija
%bidirectional = dvostranski/dvosmerni??
...
%handler = prestreznik
%pattern matching = izločitev vzorca / ujemanje vzorca??
%substitution = substitucija zamenjava ??
%await = blokada
...
Koraki v množici $t_i$ se izvedejo ob času $i$. Kadar vsaj ena množica $t_i$ vsebuje vsaj dva ali več korakov, programu pravimo da je asinhron ali paralelen. V primeru, da so $t_i = \{k_i\}$, sta izvajanji $S_p$ in $S_s$ ekvivalentni.
...
ekvivalentno prejšnjima dvema z bistveno razliko. Izvajanje $\hat{S_p}$ se konča enota časa prej, kot $S_p$ ali $\tilde{S_p}$. Dokler so pogoji izpolnjeni lahko ta postopek ponavljamo in dobimo izvajanja, ki so ekvivalentna prvotnemu, a bistveno, se hitreje končajo. Ta postopek se imenuje paralelizacija programa. Hitrejše izvajanje programa je glavni razlog, da si želimo paralelno izvajanje programa.


Ugotoviti kateri koraki so med sabo neodvisni je v večini primerov netrivialno. Programski jezik ugotovi neodvisnost le na najbolj trivialnih primerih. Običajen pristop za reševanje tega problema je zato, da se korake razdeli v procese ali niti. Za koraka $k_i$ in $k_j$ se smatra da sta neodvisna, če pripadata različnemu procesu, razen v primeru ko eksplicitno označimo, da koraka sta odvisna. Sedaj ima programski jezik na voljo veliko več trivialnih korakov, ki jih zna paralelizirati.

Seveda imamo sedaj ne trivialen postopek delitve korakov v procese. Na uporabniku je, da v izvorni kodi označi kateri koraki pripadajo kateremu procesu. Da se ta postopek olajša kolikor je mogoče, je želeno od programskega jezika, da je ta postopek čim bolj jasen in enostaven.


%Da tudi v praksi dosežemo paralelno izvajanje programa, med drugim potrebujemo programski jezik, ki to omogoča. 
Nekateri programski jeziki, predvsem starejše verzije, nimajo direktno podpore za paralelno izvajanje programa. 
...
Drugi programski jeziki kot so C++, Java... imajo kot del standardne knjižnice konstrukte, ki jim omogočajo paralelno izvajanje programa. Čeprav v ozadju mnogi jeziki uporabljajo POSIX, so detajli skriti in zato bolj naravni za programerja. 
Nekateri programski jeziki omogočajo mnoge konstrukte povezane s paralelnim programiranjem, vendar direktno ne omogočajo paralelnega izvajanja. Na primer CPython ima tako imenovan GIL (global  interpreter lock)~\cite{gil}, ki preprečuje tolmaču, da bi izvajal več kot eno nit na enkrat.
Le redki programski jeziki, recimo Go, pa so bili zasnovani z paralelnim programiranjem v mislih in imajo zato konstrukte potrebne za paralelno izvajanje vgrajene v jezik.
...
\begin{itemize}
	...
	\item konstante?? če ti paše jih dodaj
	\item Vsota predstavlja poenostavljen Variant??ja
	\item lambda??
...
%introduction
%najprej vse stare star sinteks 

%nov spawn evaluation, type, syntaks
%nov mobile type evaluation, syntak

%implementation kaj se razlikuje od teorije
%future work
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Razširjen \lae{}}\label{sec:nove-strukture}
...
\subsubsection{operacijska semantika prej}
