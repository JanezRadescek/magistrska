...
Izraze v \lae razdelimo na vrednosti in izračune. Pripadajočo sintakso vidimo na sliki~\ref{fig:vrednosti}.
...
\begin{figure}[tp]
	\parbox{\textwidth}{
		...
		\bnfor& \tmletrec[: \tyfun{X}{\tycomp{Y}{(\o,\i)}}]{f}{x}{M}{N} & &\text{rekurzivna definicija} \\
		\bnfor& V\,W                                    & &\text{aplikacija} \\
		\bnfor& \tmmatch{V}{\tmpair{x}{y} \mapsto M}    & &\text{izločitev produkta} \\
		\bnfor& \tmmatch[\tycomp{Z}{(\o,\i)}]{V}{}                        & &\text{je to sploh dovoljeno?} \\
		...
		\bnfor& \tmwith{op}{x}{M}{p}{N}      & &\text{obljuba} \\
		\bnfor& \tmawait{V}{x}{M}             & &\text{prestreznik prekinitve}
		\end{align*}
	} 
