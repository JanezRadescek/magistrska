...
\begin{proof}
	...
	\begin{itemize}
		...
		Ker $\op \neq \op'$, velja $(\o'', \i'') \order{O \times I} (\pi_2(\opincomp{op'}{(\o', \i')}))(op)$, po lastnosti~\ref{lem:eo-4} iz leme~\ref{lem:effects-order}. Po pravilu \rulename{TyComp-Subsume} ima $N_1$ tip $(\pi_2(\opincomp{op'}{(\o', \i')}))(op)$.
...
\begin{proof}
	...
	\begin{itemize}
		...
		Po indukcijski predpostavki ima $Q'$ tip $C'$.
		Po pravilu \rulename{TyProc-Interrupt} ima $\tmopin{op}{V}{Q'}$ tip $\opincomp{op}{C'}$. Če je $C' = C''$, potem je $C$ naredil korak \rulename{TyRedu-Same}. Če smo $C'$ dobili z \rulename{TyRedu-Change} potem 
	\end{itemize}
