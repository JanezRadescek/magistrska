...
Račun \lae{} je razširitev $\lambda$-računa. Računu \lae{} dodamo izraze, ki poosebijo bistvo asinhrono vzporednega programiranja z pomočjo učinkov. Da \lae{}-račun nekoliko približamo vsakdanjim programskim jezikom, mu dodamo tudi izraze za  naravna števila, par, ujemanje vzorca... Še vedno pa izpustimo veliko izrazov, ki jih sicer imamo v jeziku \aeff{}.
...
Nova vrednost je \emph{obljuba}\footnote{Tudi nekateri drugi programski jeziki kot na primer Scala imajo obljube~\cite{scala}. Kljub temu pa običajno ne v navezi z učinki.}. Je vrednost, ki jo je vrnil prestreznik in jo posebej označimo, da ji lažje pravilno določimo tip.
...
Osnovni proces je imenovan \emph{Zagon}. Vsebuje izračun $M$, ki se izvaja.
\emph{Vzporeden proces} vsebuje dva procesa. Vzporedne procese lahko gnezdimo in tako dobimo drevo procesov, katerega listi so zagoni. Na ta način imamo lahko kot del programa več procesov, ki se potencialno izvajajo hkrati.
\emph{Signal procesa} in \emph{prekinitev procesa} delujeta enako kot signal izračuna in prekinitev izračuna, le da delujeta na procesih. Njuna naloga je, da preneseta učinke iz enega procesa na preostale.
...
Množica $I$ je nekoliko bolj zakomplicirana. Ko prestreznik ujame prekinitev, se bo nadaljevanju dodal izračun obljuba iz prestreznika. Slednji lahko vsebuje nove signale in prestreznike. Posledično lahko sedaj spremenjeni izračun začne oddajati nove signale in prestrezati nove prekinitve. Toda tudi novi prestrezniki lahko ponovno namestijo nove signale in prestreznike in tako naprej.
...
Nato naredimo ustrezno unijo $\i \cup \i'$, da dodamo nove učinke iz obljub.
...
Na tipih učinkov imamo urejenost. Na $O$ je urejenost dana z relacijo podmnožica $\subseteq$.
Urejenost na $I$ pa je dana z sledečo relacijo.
$$
\begin{array}{l c l}
	\i \order I \i'
	&
...
Prvi lastnost nam pove, da ko prekinitev sproži vse prestreznike to ne vpliva že obstoječe signale, kvečjemu lahko dobimo kakšen nov signal.
Tretja lastnost pravi, da ko operacija sproži vse prestreznike se bodo signali in prestrezniki v izračunih obljuba "pridružili" že obstoječim.
Četrta lastnost pravi, da prekinitev ne vpliva na prestreznike z drugačnim imenom operacije.

\begin{lema}\label{lem:effects-order}
	\begin{enumerate}
		\item $o \order{O} \pi_1 (\opincomp{op}{(\o, \i)})$
		\item $i \order{I} (i' \cup i)$
		\item Če $\i(\op) = (\o', \i')$, potem $(\o', \i') \order{O \times I} \opincomp{\op}{(\o, \i)}$
		\item Če $\op \neq \op'$ in $\i(\op') = (\o', \i')$, potem $(\o', \i') \order{O \times I} (\pi_2(\opincomp{\op}{(\o, \i)}))(\op')$
		\item Če $(\o, \i) \order{O \times I} (\o', \i')$, potem $\opincomp{op}{(\o, \i)} \order{O \times I} \opincomp{op}{(\o', \i')}$
	\end{enumerate}
\end{lema}
...
\begin{proof}
	\begin{enumerate}
		...
		\item V pogoju definicije urejenosti ločimo dva primera. Za $\op$ za katere velja $i(op) = \bot$, je pogoj na prazno izpolnjen. Poglejmo sedaj tiste $\op$ za katere velja $i(op) = (\o'', \i'')$. 
		Če $i'(op) = \bot$, tedaj je $(\i' \cup \i)(op) = (\o'', \i'')$ in pogoj iz definicije $\order{I}$ velja.
		Če $i'(op) = (\o''', \i''')$, tedaj je $(\i' \cup \i)(op) = (\o'' \cup \o''', \i'' \cup i''')$.
		Po indukcijski predpostavki pogoj iz definicije $\order{I}$ ponovno velja.
		\item Če $\i(\op) = (\o', \i')$ potem $\opincomp{op}{(\o, \i)} = (\o \cup \o' , \i[\op \mapsto \bot] \cup \i')$. Očitno velja $\o' \order{O} (\o \cup o')$.
		Prav tako velja $\i' \order{I} (\i[\op \mapsto \bot] \cup \i')$ po drugi točki te leme.
		...
		Posledično velja $(\o', \i') \order{O \times I} (\i[\op \mapsto \bot] \cup \i'')(\op')$, ker je $\i[\op \mapsto \bot] \order{I} \i[\op \mapsto \bot] \cup \i''$ in upoštevajoč definicijo $\order{I}$. Ker $(\i[\op \mapsto \bot] \cup \i'')(\op') = (\pi_2(\opincomp{\op}{(\o, \i)}))(\op')$ je lastnost dokazana.
		Druga možnost je $\i(op) = \bot$. Tedaj je $\opincomp{op}{(\o, \i)} = (\o, \i)$ in trditev sledi. 
...
Prestreznik ima isti tip kot njegovo nadaljevanje. Preverimo tudi, da je $\op$ ena izmed operacij, ki jo lahko prestrežemo in da ima izračun obljuba tip obljuba z učinki $\i(\op) = (\o', \i')$.
...
$$\opincomp{op}{\tyrun{A}{\o, \i}} \defeq \tyrun{A}{\opincomp{op}{(\o, \i)}} \qquad 
...
\begin{lema}\label{lem:inversion-lema}
	...
	\begin{itemize}
		...
		\item Če je $M$ oblike $\tmopin{op}{V}{N}$, potem ve1lja $\Gamma \types V \of B$, $\Gamma \types N \of \tycomp{A}{\o'', \i''}$ in $\opincomp{op}{(\o'', \i'')} \order{O, I} (\o, \i)$.
...
\begin{proof}
	...
	\begin{itemize}
		...
		Po pravilu \rulename{TyComp-Signal} ima $M'$ tip $\tycomp{A}{\opincomp{op}{(\o'', \i'')}}$.
		Po lem~\ref{lem:effects-order} velja $\opincomp{op}{(\o'', \i'')} \order{O \times I} \opincomp{op}{(\o', \i')}$.
		...
		Ker je $\i'(op) = (\o'', \i'')$, velja $(\o'', \i'') \order{O \times I} \opincomp{op}{(\o', \i')}$ po lemi~\ref{lem:effects-order}.
...
\begin{proof}
	...
	\begin{itemize}
		...
		Po pravilu \rulename{TyProc-Par} ima $\tmpar{Q_1'}{Q_2}$ tip $\typar{C_1'}{C_2}$, kjer je $C$ naredil korak \rulename{TyRedu-Par}.
