...
V nadaljevanju bomo uporabljali substitucije.  Substitucije na vrednostih in izračunih definiramo, kot vidimo na sliki~\ref{fig:substitucije}. Sledimo dogovoru, da če se v nekem kontekstu pojavijo vezane spremenljivke, jih izberemo drugače od prostih spremenljivk~\cite{barendregt}.
...
Račun \lae{} opremimo z operacijsko semantiko malih korakov, ki je definirana z relacijo \emph{korakov} $M \reduces N$. Koraki za izračune so podani na sliki~\ref{fig:small-step-semantics-of-computations}. 
...
Večina vrednosti dobi standardne tipe, kot so naravno število, enotski tip, produkt, vsota in funkcijski tip. 
...
kjer je $\Rightarrow$ potenciranje, $\times$ kartezični produkt in $(-)_\bot$ dvig. Dvig je disjunktna unija množice z $\bot$.
...
\begin{proof}
	\begin{enumerate}
		...
		Naj bo sedaj $\i(\op) = (\o'',\i'')$. Tedaj sta iskana $\o'''$ in $\i'''$ iz definicije ravno $\o''$ in $\i''$. $\o''\order{O}\o''$ velja, ker je $\order{O}$ delna urejenost. Po koindukcijski predpostavki velja $\i'' \order{I} \i''$.
...
Sledeča lema pravi, da je $\cup$ na $I$ simetrična operacija, spoštuje $\order{I}$ in da je $I$ $\cup$-polmreža.
...
\begin{proof}
	\begin{enumerate}
		...
		Simetrično za $\op$ za katere je $\i_2(\op) = \bot$.
		Naj bo sedaj $\i_1(\op) = (\o_1', \i_1')$ in $\i_2(\op) = (\o_2', \i_2')$ in $\i_3(\op) = (\o_3', \i_3')$. Ker je $O$ z $\order{O}$ $\cup$-polmreža velja $(\o_1' \cup \o_2') \order{O} \o_3'$. Ker je $\i_1' \order{I} \i_3'$ in $\i_2' \order{I} \i_3'$ je po koindukcijski predpostavki $(\i_1' \cup \i_2') \order{I} \i_3'$.
	\end{enumerate}
...
Za tip $\tycomp{A}{\o, \i}$ rečemo, da je \emph{podtip} tipa $\tycomp{A}{\o', \i'}$, če velja $(\o, \i) \order{O \times I} (\o', \i')$. Če lahko izračunu določimo nek podtip $\tycomp{A}{\o, \i}$ tipa $\tycomp{A}{\o', \i'}$, potem ima izračun tudi tip $\tycomp{A}{\o', \i'}$.
...
je ključno da tovor $V$ ne vsebuje spremenljivke $p$, saj v drugem izračunu spremenljivka $p$ v vrednosti $V$ nima tipa. Da zagotovimo, da vrednost $V$ ne vsebuje spremenljivke $p$, omejimo tipe, ki pripadajo operacijam, na osnovne tipe. Vrednosti, ki imajo osnovni tip, namreč ne morejo vsebovati obljub. To je močna omejitev, funkcijski tip na primer namreč ni osnovni. To omejitev bomo v poglavju~\ref{sec:razsirjen-lae} spremenili.
...
Kadar ne dokazujemo, na drevo izpeljave za tip izračuna, ne vemo, katero pravilo točno je bilo zadnje uporabljeno. Posledično ne moremo uporabiti predpostavk pravila, ki bi nam povedale o tipih komponent, kljub temu, da imamo tip celega izračuna.
...
Ko proces naredi sledeči korak 
$$\tmpar{\tmopout{op}{V}{\tmrun{M_1}}}{\tmrun{M_2}} \reduces \tmopout{op}{V}{\tmpar{\tmrun{M_1}}{\tmopin{op}{V}{M_2}}},$$ 
bo pripadajoči tip naredil korak
...
Potencialno spremembo tipa najprej formaliziramo s pravili na osnovnih tipih, kot jih lahko vidimo na sliki~\ref{fig:process-type-reductions-basic}. Nato jih posplošimo še za splošen tip na sliki~\ref{fig:process-type-reductions}.
