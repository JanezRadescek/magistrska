...
\begin{proof}
	...
	Če je uporabljeno pravilo $TyComp-Return$, potem je $M = \tmreturn{V}$ in po pravilu $R-Return$ sledi, da je $M$ delni rezultat in velja (ii).
	Če je uporabljeno pravilo $TyComp-Apply$, potem je $M = V W$ in $V$ mora biti oblike $\tmfunano{x}{M}$ ali $\tmfunrecano{f}{x}{M}$. Posledično lahko naredimo korak v $M[W/x]$ ali $M[V/x, (\tmfunrec{f}{x \of X}{M})/f]$.
	...
	Izračun $N_1$ ima tip in po $IP$ lahko naredi korak ali pa je delni rezultat. Če $N_1$ lahko naredi korak v $N_1'$, potem lahko po pravilu za evalvacijo v okolju tudi $M$ naredi korak.
	Če pa je $N_1$ delni rezultat ločimo tri možnosti. Če je $M = return V$, potem lahko naredimo korak v izračun $N[V/x]$. V drugih dveh primerih lahko uporabimo algebraičnost prestreznika ali blokade in naredimo ustrezen korak. 
	
	Če je zadnje pravilo $TyComp-MatchPair$, potem je $M$ enak $\tmmatch{V}{(x,y) \mapsto N}$. Ker ima vrednost $V$ tip $X \times Y$, je $V$ oblike $(W_1,W_2)$ in posledično lahko naredimo korak v izračun $N[W_1/x,W_2/y]$.
	
	Zadnje pravilo ne more biti $TyComp-MatchEmpty$, saj bi potem morali imeti vrednost tipa $0$. Vrednost tega tipa ne obstaja.
	
	Če je zadnje pravilo $TyComp-MatchSum$, potem je $M$ enak $\tmmatch{V}{\tminl{x} \mapsto N_1, \tminr{y} \mapsto N_2}$. Ker ima vrednost $V$ tip $X + Y$, je $V$ oblike $\tminl{W_1}$ ali $\tminr{W_2}$. Če je oblike $\tminl{W_1}$ lahko naredimo korak v $N_1[W_1/x]$. Sicer je oblike $\tminr{W_2}$ in lahko naredimo korak v $N[W_2/y]$.
