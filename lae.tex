...
\begin{figure}[H]
	...
	\begin{mathpar}
		...
		(\tmfunano{x}{M})[V/x] = \tmfunano{x}{(M[V/x])} \\ (\tmfunrecano{f}{x}{M})[V/x] = \tmfunrecano{f}{x}{(M[V/x])} \\
...
\mP{For motivation, you can introduce result forms (Slika 10) after operational semantics rules and then give a simple example of a term that is not well typed that does not terminate in a result. }
...
Vsaki operaciji priredimo nek tip, kot vidimo na sliki~\ref{fig:tipi}. Ker izračun 
...
Izrek o varnosti, ki je sestavljen iz izreka o ohranitvi in izreka o napredku nam zagotavlja, da ne moremo dobiti napak ob izvajanju, če imamo tipe, kot smo jih definirali.

\jR{Add lemma about finality of results/semiresults}
...
\begin{proof}
	Predpostavimo, da je $V$ spremenljivka. Edino pravilo za določitev tipa spremenljivke je \rulename{TIP-VAR}.
...
\begin{proof}
	...
	\begin{itemize}
		...
		Po pravilih \rulename{R-Return} in \rulename{R-Comp} sledi, da je $M$ rezultat in velja (ii).
		...
		\item Če je zadnje pravilo \rulename{TyComp-Subsume}, lahko $IP$ uporabimo direktno na $M$ in posledično lahko $M$ naredi korak ali pa je $M$ rezultat.
...
%		\item Če je zadnje pravilo \rulename{Ty-Cons-F}, potem je $W$ enak $false$ in $B$ enak $bool$. Ker je $W[V/x] = false[V/x] = false$, ima $W[V/x]$ tip $bool = B$, po pravilu \rulename{Ty-Cons-F}.
		...
		\item Če je zadnje pravilo \rulename{TyVal-Pair}, potem je $W =\tmpair{W_1}{W_2}$ in $B = B_1 \times B_2$. Po $IP$ imata $W_1[V/x]$ in $W_2[V/x]$ tip $B_1$ in $B_2$.
		Ker je $W[V/x] = \tmpair{W_1[V/x]}{W_2[V/x]}$, ima vrednost $W[V/x]$ tip $\typrod{B_1}{B_2} = B$ po pravilu \rulename{TyVal-Pair}.
		
		\item Če je zadnje pravilo \rulename{TyVal-Inl}, potem je $W = \tminl{W_1}$ in $B = B_1 + B_2$. Po $IP$ ima $W_1[V/x]$ tip $B_1$.
		Ker je $W[V/x] = \tminl{W_1[V/x]}$, ima vrednost $W[V/x]$ tip $B_1 + B_2 = B$ po pravilu \rulename{Ty-Sum}.
		
		\item Če je zadnje pravilo \rulename{TyVal-Inr}, potem je $W = \tminr{W_2}$ in $B = B_1 + B_2$. Po $IP$ ima $W_2[V/x]$ tip $B_2$.
		...
		\item Če je zadnje pravilo \rulename{TyVal-Promise}, potem je $W = \tmpromise{W'}$ in $B = \typromise{B'}$. Po $IP$ ima $W'[V/x]$ tip $B'$.
...
\begin{proof}
	...
	\begin{itemize}
		\item Če je zadnje pravilo \rulename{TyVal-Var'}, potem je $V = y$. Ker $\typromise{A}$ ni osnoven tip, velja $y \in \Gamma_1$ ali $y \in \Gamma_2$.
...
\begin{proof}
	...
	\begin{itemize}
		\item Če je zadnje pravilo \rulename{TyVal-Var}, potem je $V = x$ in velja $x \in \Gamma_1$ ali $x \in \Gamma_2$. Po pravilu \rulename{TyVal-Var} velja $\Gamma_1, \Gamma_3, \Gamma_2 \types V \of A$.
...
%		\item Če je zadnje pravilo \rulename{Ty-Cons-F}, potem je $V = false$. Po pravilu \rulename{Ty-Cons-F} velja $\Gamma_1, \Gamma_3, \Gamma_2 \types V \of A$.
		...
		\item Če je zadnje pravilo \rulename{TyVal-Promise}, potem je $V = \tmpromise{V'}$. Po $IP$ ima $V'$ isti tip v razširjenem kontekstu.
