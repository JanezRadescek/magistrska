...
\emph{Signal} vsebuje operacijo $\op$, pripadajočo vrednost $V$ imenovano \emph{tovor} in nadaljevanje $M$. Signal se razširi navzgor po izračunih, dokler ne pride na nivo procesov. Nakar se bo razširil v vse druge procese in ob tem spremenil v \emph{prekinitev}. Prekinitev je simetrična signalu, prav tako ima operacijo $\op$, vrednost $V$ in nadaljevanje $M$. Le da prekinitev potuje po izračunih navznoter namesto navzgor. Na ta način smo informacijo iz $V$ prenesli iz enega procesa na vse preostale in tako imamo vzpostavljeno komunikacijo med procesi. Da imamo asinhrono izvajanje lahko, namesto da se signal in prekinitev širita, delamo korake tudi v njunem nadaljevanju $M$.
Informacijo v prekinitvi prestrežemo s pomočjo \emph{prestreznika}. Prestreznik je sestavljen iz operacije $\op$, spremenljivke $x$, izračunom $M$ imenovanim \emph{obljuba}, spremenljivko $p$ prav tako imenovano \emph{obljuba} in nadaljevanje $N$.\footnote{Izračun obljuba bo vrnil posebno vrednost obljuba, ki jo bomo substituirali v spremenljivko obljuba.}
Običajno se izvaja nadaljevanje $N$, med tem ko preostali deli čakajo. Ko pride prekinitev z istim imenom operacije se spremenljivka $x$ substituira z tovorom $V$ v obljubi $M$. Nakar se prestreznik spremeni v zaporedje izračuna $M$ in prekinitve, ki ima za kontinuacijo izračun $N$. Rezultat izračuna $M$ bo vezan v spremenljivko $p$ in se ga tako lahko uporabi v $N$. Ko prestreznik ujame ustrezno prekinitev rečemo, da se je izpolnila obljuba $p$.
...
\begin{figure}[H]
	...
	\begin{mathpar}
		...
		(\tmfunano{y}{M})[V/x] = \tmfunano{y}{(M[V/x])}\ {\color{rulenameColor}(y \neq x)} \\
		(\tmfunrecano{f}{y}{M})[V/x] = \tmfunrecano{f}{y}{(M[V/x])}\ {\color{rulenameColor}(y \neq x)} \\
...
Signal in prekinitev na procesih se obnašata enako kot na izračunih. Par procesov lahko naredi korak, če lahko eden izmed procesov naredi korak. Koraki za procese so podani na sliki~\ref{fig:small-step-semantics-of-processes}.
...
Da se izognemo programom, ki se bi ustavili v neželeni obliki, uvedemo sistem tipov.
...
Ko proces ali izračun od zunaj dobi novo prekinitev, se mu lahko spremeni tip učinkov.
...
Signal ima isti tip kot njegova kontinuacija. Sproti pa preverimo, da je operacija $\op$ v $o$ in da ima tovor tip, ki pripada operaciji.
Ker izračun 
...
Prekinitev ima tip, ki ga dobimo tako, da na tipu kontinuacije uporabimo preslikavo $\opincomp{op}$. Prav tako preverimo, da ima tovor tip, ki pripada operaciji $\op$. 
Prestreznik ima isti tip kot kontinuacija. Preverimo tudi, da je $\op$ ena izmed operacij, ki jo lahko prestrežemo in da ima izračun obljuba tip obljuba z učinki $\i(\op) = (\o', \i')$.
Blokada ima isti tip kot njegova kontinuacija. Preverimo tudi da ima vrednost $V$ tip obljuba. 
Preostali izračuni imajo pričakovane tipe.

Na tipih učinkov imamo urejenost. Na $O$ je urejenost dana z relacijo podmnožica $\subset$.
Na $I$ pa je dana z sledečo relacijo.
$$
\begin{array}{l c l}
	\i \order I \i'
	&
...
Tipi procesa zagon je odvisen od tipa pripadajočega izračuna in posledično vsebuje tipe učinkov. 
Signal procesa ima isti tip, kot njegova kontinuacija. 
Podobno, kot se spremeni tip prekinitve izračuna, se spremeni tip prekinitve procesa.
...
Izrek o ohranitvi pove da, če izračun, ki ima tip, naredi korak v nek drug izračun bo pri tem ohranil tip.
...
Najprej dokažemo trditev o napredku samo za izračune. Da bomo lahko uporabili indukcijsko predpostavko na kontinuacijah prestreznikov, v kontekstu dovolimo spremenljivke tipa obljuba.
