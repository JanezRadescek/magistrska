...
Čeprav lahko v jeziku \aeff{} napišemo že veliko zanimivih primerov\footnote{Članek~\cite{aeff} omeni kar nekaj primerov.}, se še zmeraj pojavijo določeni problemi.
...
Te tri probleme bomo sedaj enega za drugim razrešili.
...
\begin{figure}[H]
	...
	\begin{align*}
		...
		\text{kjer je }R = \;& \tmfunano{y}{(\tmwithrec{op}{r}{x}{M}{p}{\tmreturn{p}})} \\
...
Da zagotovimo, da ne bi kakšni vrednosti, ki vsebuje zunanjo obljubo, dodelili zavit tip, nekoliko spremenimo pravilo za izračun tipa spremenljivke in dodamo pravili za zavito vrednost in izračun odvijanje. Spremebe prikaže slika~\ref{fig:tipi-pravila-prenosljivi}.
Okolje je imelo že od začetka določen vrstni red, vendar šele sedaj to postane zares pomembno. Ko določamo tip zavite vrednosti, v okolje dodamo $\bb$. S tem za v bodoče označimo katere spremenljivke so zunanje ali notranje glede na zavito vrednost. Kadar ima vrednost kontekst $\Gamma_1, \bb, \Gamma_2$ pravimo, da ima vrednost \emph{prenosljiv pogled} na kontekst $\Gamma_1$. 
...
Ker se izračun $M$ v $\tmspawn{M}{N}$ lahko razširi mimo prestreznikov moramo paziti, da ima $M$ prenosljiv pogled in je posledično varen.   
