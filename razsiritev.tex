...
Imamo dve funkciji, $\mathsf{callWith}$ in $\mathsf{remote}$.
Funkcija $\mathsf{remote}$ sprejme seznam funkcij. Ko bo prišla prekinitev $\mathsf{call}$, bo iz seznama vzela ustrezno funkcijo, izračunala vrednost te funkcije v dani vrednosti $\mathsf{x}$ in poslala nazaj signal z id-jem klica in rezultatom. Na koncu pa bo s pomočjo rekurzivne pomožne funkcije ponovno namestila prestreznik.
Funkcija $\mathsf{callWith}$ sprejme referenco (ki bo predstavljala unikatno številko klica), id funkcije (ki jo želimo poračunati) in vrednost (v kateri želimo izračunati funkcijo).
Nato pošlje signal $\mathsf{call}$ z ustreznimi vrednstmi in poveča številko v referenci. Namesto da sama funkcija čaka na rezultat, bo vrnila pomožno funkcijo, ki bo čakala na rezultat (s pravo številko klica).  
V prvem procesu ustvarimo referenco in s pomočjo $\mathsf{callWith}$ začnemo računati dve pomožni vrednosti, na kateri pa ne čakamo, da se izračunata, ampak nemudoma začnemo z računanjem vrednosti $\mathsf{z}$. Šele nato začnemo po potrebi čakati na prvi dve vrednosti in končno vse tri vrednosti seštejemo.
V drugem procesu začnemo funkcijo $\mathsf{remote}$ s seznamom dveh funkcij.
...
let callWith callCounter fNo x (*@$\defeq{}$@*)
...
run	let callCounter (*@$\defeq{}$@*) ref 0 in
    let y1 (*@$\defeq{}$@*) callWith callCounter 0 2 in
    let y2 (*@$\defeq{}$@*) callWith callCounter 1 3 in
    let z (*@$\defeq{}$@*) 42 + 7 in
    return y1 () + y2 () + z
...
Tudi pravilo za tip rekurzivnega prestreznika je zelo podobno prvotnemu prestrezniku. Spremenljivka $r$ dobi tip $\tyfun{\tyunit}{\tycomp{\typromise{B}}{\emptyset, \op \mapsto (\o', \i')}}$, ki lepo predstavlja izračun, ki ne sproži nobenega signala in prestreže le operacijo $\op$.
...
let callWith (*@$\defeq{}$@*)
	fun callCounter fBoxed x ->
		...
		callCounter := callNo + 1;
		promise (result (callNo', y) (*@\hlyellow{$r$}@*) ->
			if callNo = callNo' then
				return <y>
			else
				(*@\hlyellow{$r$}@*) ()
...
let remote fList (*@$\defeq{}$@*)
	promise (call (callNo, fNo, x) (*@\hlyellow{$r$}@*) ->
		let f (*@$\defeq{}$@*) nth fList fNo in
		let y (*@$\defeq{}$@*) f x in
		send result (callNo, y);
		(*@\hlyellow{$r$}@*)()
...
run	let callCounter (*@$\defeq{}$@*) ref 0 in
    let y1 (*@$\defeq{}$@*) callWith callCounter (*@\hlyellow{$[fun x -> 2 * x]$}@*) 2 in
    let y2 (*@$\defeq{}$@*) callWith callCounter (*@\hlyellow{$[fun x -> 7 * x]$}@*) 3 in
    let z (*@$\defeq{}$@*) 42 + 7 in
    return y1 () + y2 () + z
...
Sedaj ko imamo dinamične procese, lahko obe evalvaciji funkcije $\mathsf{f}$ v $\mathsf{remote}$ opravimo hkrati v svojem procesu, kar potencialno pohitri izvajanje programa. Primer~\ref{prog:razsiritev-2} lahko sedaj spremenimo v~\ref{prog:razsiritev-3}. 
...
run	let callCounter (*@$\defeq{}$@*) ref 0 in
let y1 (*@$\defeq{}$@*) callWith callCounter [fun x |-> 2 * x] 2 in
let y2 (*@$\defeq{}$@*) callWith callCounter [fun x |-> 7 * x] 3 in
let z (*@$\defeq{}$@*) 42 + 7 in
return y1 () + y2 () + z
...
Tudi za vse dinamične procese si želimo da se razširijo do vrha. Zato rezultatom za izračune iz slike~\ref{fig:results-rules} dodamo še sledeč rezultat. 
...
\begin{proof}
	...
	\begin{itemize}
		...
		Ker velja $(\o'', \i'') \order{O, I} \i'(op)$, velja $(\o'', \i'') \order{O \times I} \opincomp{op}{(\o', \i')}$ in posledično ima po pravilu \rulename{TyComp-Subsume} in lemi~\ref{lem:substitucija-izračuni-2} izračun $N_1[V/x]$ tip $\tycomp{\typromise{B}}{\opincomp{op}{(\o', \i')}}$.
